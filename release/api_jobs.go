...
func (a *JobsAPIService) CleanupTerminalJobsExecute(r ApiCleanupTerminalJobsRequest) (int32, *http.Response, error) {
	...
	if r.principal != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "principal", s.Index(i).Interface(), "form", "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "principal", t, "form", "multi")
		}
	}
	if r.origin != nil {
		t := *r.origin
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "origin", s.Index(i).Interface(), "form", "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "origin", t, "form", "multi")
		}
	}
	if r.executor != nil {
		t := *r.executor
